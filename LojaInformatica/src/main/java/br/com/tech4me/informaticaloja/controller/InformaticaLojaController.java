...
@PostMapping
public ResponseEntity<InformaticaLojaDtoCompleto> cadastrarpPc(@RequestBody @Valid InformaticaLojaDtoCompleto pc){
  return new ResponseEntity<>(servico.cadastrarPc(pc), HttpStatus.CREATED); 
}

@GetMapping
    public ResponseEntity<List<InformaticaLojaDtoCompleto>> MostrarListaDePeças() {
